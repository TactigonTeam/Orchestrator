...
from Braccio_Orchestrator import Braccio_Msg_Command_List, Braccio_Msg_Command
from Macro_Braccio import Macro

class Student_Braccio:
    RUNNING = 1
    STOPPED = 3
    TIMEOUT_S = 15
    ...
    def __init__(self, robot_pipe: Connection, voice_pipe: Connection):
        """Student loads a Macro and send it's command to the Braccio object

        Args:
            robot_pipe (Connection): Connection to send commands to braccio
        ...
        while state == self.RUNNING:
            ...
            if mode == self.MODE_COMMAND:
                ...
                command_msg = Braccio_Msg_Command_List([
                    Braccio_Msg_Command(
                        command.x,
                        command.y,
                        command.z,
                ...
                if mode == self.MODE_WAIT:
                    if returned_command_msg == command_msg:
                        ...
                        if returned_command_msg.has_error:
                            ...
                            lib.user_feedback(lib.FeedbackEnum.STUDENT_BRACCIO_HAS_ERROR)
                        ...
                        lib.user_feedback(lib.FeedbackEnum.STUDENT_BRACCIO_COMMAND_MISMATCH)
                    

            if voice_pipe.poll():
                phrase = voice_pipe.recv()
                if phrase == phrase_stop:
                    robot_pipe.send(Braccio_Msg_Command.home())
                    state = self.STOPPED
                # else:
